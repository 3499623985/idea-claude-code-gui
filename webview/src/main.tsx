 ...
 */
/**
 * JCEF can keep stale layout/compositor state after the IDE resumes from the
 * background. Refresh the font scale variable and force a light layout pass so
 * components relying on viewport measurements recalculate.
 */
function setupScaleRecovery() {
  const getExpectedScale = (): string => {
    const fromInline = document.documentElement.style.getPropertyValue('--font-scale').trim();
    if (fromInline) return fromInline;

    return String(getFontScaleForLevel(getSavedFontScaleLevel()));
  };
  ...
  const forceReapply = (reason: string) => {
    ...
    applyAppTypographyVariables(document.documentElement, scale);
    applyMessageTypographyVariables(document.documentElement, latestEditorFontConfig?.fontSize, scale);
...
function applyEditorTypographyConfig(config: {
  ...
  applyMessageTypographyVariables(root, config.fontSize);
